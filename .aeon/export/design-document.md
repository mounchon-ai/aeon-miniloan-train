...
| ADR-003 | design · revised | 2026-09-04T06:00:00Z | dev เปิด GAP-miniloan-001 (พบระหว่าง /dev:plan): sitemap.json ทั้ง 14 โหนดยังไม่มีฟิลด์ app เลย ทำให้ DV19 (gate 96) ตรวจไม่ผ่านว่า shell ของแอป web เอื้อมถึงทุกหน้าจอ — เจ้าของโปรเจกต์ยืนยันแล้วว่าทั้ง 14 หน้าเป็นของแอป web ทั้งหมด (apps/api ไม่มีหน้าจอของตัวเอง เป็น backend ล้วน) การเปลี่ยนนี้เป็นการเติมฟิลด์ app ที่ schema ของ sitemap.mjs รองรับอยู่แล้วแต่ไม่เคยถูกกรอก ไม่เปลี่ยนพฤติกรรม วัตถุประสงค์ หรือสิทธิ์ของหน้าจอใด ๆ | รับเข้าแล้ว 2026-09-04T07:24:57Z |
| ADR-004 | design · revised | 2026-09-05T00:00:00Z | design ตัดสินเอง (ไม่มีใบเปลี่ยนแปลงจาก req) — DQ-miniloan-008 ถามว่าขอบเขต transaction อยู่ตรงไหนเมื่อเบิกจ่ายสำเร็จแต่สร้างตารางผ่อนไม่สำเร็จ เจ้าของตัดสินว่า atomic ทั้งคำสั่ง: เบิกจ่าย -> สร้าง LoanAccount -> สร้าง RepaymentSchedule และ Installment ทุกงวด อยู่ในธุรกรรมเดียว ล้มเหลวคือ rollback ทั้งก้อน ไม่มีสถานะครึ่งทาง แล้วผู้ใช้สั่งซ้ำเองด้วย requestId เดิม กันซ้ำที่ ENT-012 ด้วย unique constraint ที่ฐานข้อมูล การตัดสินนี้ไม่ได้เปลี่ยนพฤติกรรมที่ตกลงไว้ แต่เป็นการเขียนลงในช่องที่มีไว้ถือมันโดยเฉพาะ คือ interfaces.json ruleEnforcement ของ BR-miniloan-015@v1 ซึ่งเดิมระบุแค่ domain และ note ไม่ได้พูดถึงขอบเขตธุรกรรมเลย — SCN-miniloan-035 และ SCN-miniloan-036 ที่เซ็นไปแล้วบังคับผลลัพธ์นี้อยู่ก่อนแล้ว (ล้มทันที ไม่มี retry อัตโนมัติ และสั่งซ้ำแล้วต้องได้บัญชีเดียว ไม่เกิดบัญชีที่สอง) จึงไม่มีขั้น scenario หรือ function ใดต้องทำใหม่ \|\| แก้ไขเมื่อ 2026-09-05 ระหว่างรัน /design:sitemap: ใบนี้เคยพ่วงการ์ด GAP-miniloan-002 และ GAP-miniloan-003 ของ dev ไว้ด้วย พร้อมข้อความว่า design ต้องตัดสินว่ากรอบแอปมี control อะไรบ้าง สังกัดหน้าไหน — ข้อความนั้นผิด และถอนออกจากใบนี้แล้ว หลักฐานสามชิ้นจากโค้ดของ plugin เอง: (1) mock/scripts/wireframes.mjs มี navControlId = nav-<ui-id ตัวพิมพ์เล็ก> ซึ่งมินต์ testid ของลิงก์เมนูให้อยู่แล้ว (gate 69) (2) mock/scripts/html.mjs บรรทัด 823 ระบุว่า id เหล่านี้เป็น furniture ของหน้า จงใจอยู่นอก controls[] และ MV24/MV29 (gate 71) บังคับว่าทุกหน้าที่ลูกค้าเปิดต้องถูกวาดในเปลือกของผลิตภัณฑ์ (3) รายการ kind ของ design ปิดตายที่ 13 ค่าและไม่มีค่าไหนหมายถึงกรอบแอป ส่วน auth คือค่าเดียวที่บอกว่าหน้านั้นไม่มีเปลือก ดังนั้นเปลือกแอปเป็นของ mock ไม่ใช่ของ design และ design ประกาศสิ่งที่ตัวเองเป็นเจ้าของครบแล้วใน sitemap.json (label · parent · order · entry · app) ขั้น sitemap จึงไม่มีงานต้องทำใหม่ ส่วนที่ยังเหลือจริงเป็นเรื่องของเจ้าของโปรเจกต์ตัดสิน ไม่ใช่ของ design: roster ของ dev (build.mjs wireframeControls) อ่านเฉพาะ controls[] ของ wireframe จึงมองไม่เห็น furniture id ของ mock ทำให้ dev บันทึก nav-ui-miniloan-002 ลง manifest.ui_controls ไม่ได้โดยไม่ชน DV13 สาขา e และ DV17 | รับเข้าแล้ว 2026-09-05T14:35:20Z |
| ADR-005 | design · revised | 2026-09-06T09:10:00Z | design ตัดสินเอง (ไม่มีใบเปลี่ยนแปลงจาก req — change-set.json ทั้ง 5 ใบถูกบันทึกและ absorb ครบแล้ว) — dev เปิด GAP-miniloan-005 ระหว่าง /dev:plan: rbac.json ประกาศ ACL-014 ให้ ROLE-004 (Operations) ทำ action "บันทึกการชำระยอดปิดบัญชีก่อนกำหนด" บน UC-miniloan-016 พร้อม enforceAt: [api, domain] แต่ interfaces.json ไม่เอ่ยถึง UC-miniloan-016 เลยสักครั้ง และไม่มี endpoint ใบไหนรับ action นี้ ⇒ ชั้น api ที่ ACL-014 สั่งให้บังคับใช้ ไม่มีที่อยู่ให้บังคับได้จริง · endpoint ที่มีอยู่แล้วสองใบรับแทนไม่ได้: API-015 (GET /loan-accounts/{id}/payoff-quote) เป็นการ 'ขอยอด' ของ UC-miniloan-015 ไม่ใช่การรับชำระ และ API-014 (POST /loan-accounts/{id}/payments) ประกาศ purpose ว่า 'บันทึกยอดชำระของงวดที่ค้าง' ซึ่งเป็นการชำระรายงวด โดยค่าธรรมเนียม 1% ของมันคิดจากส่วนเกินตาม BR-miniloan-046@v2 ขณะที่ยอดปิดบัญชีก่อนกำหนดคิด 1% จากเงินต้นคงเหลือตาม BR-miniloan-022@v1 — ฐานการคำนวณคนละตัว API-014 จึงรับ action นี้ไม่ได้โดยไม่แก้สัญญาของตัวเอง · การแก้คือประกาศ endpoint ของ UC-miniloan-016 ลง interfaces.json พร้อม ruleEnforcement ของ BR-miniloan-022@v1 และต่อ edge FUN-miniloan-011 exposedAs <API ใหม่> กับ UI-miniloan-005 calls <API ใหม่> · ไม่มีพฤติกรรม สิทธิ์ หรือเกณฑ์รับใบไหนเปลี่ยน — AC-miniloan-005 กับ ACL-014 บังคับผลลัพธ์นี้อยู่ก่อนแล้ว สิ่งที่ขาดคือช่องที่มีไว้ถือที่อยู่ของมันโดยเฉพาะ ซึ่งเป็นรูปเดียวกับ ADR-004 ที่เขียนขอบเขตธุรกรรมของ BR-miniloan-015@v1 ลง interfaces.json · หน่วยที่ถูกบล็อกอยู่คือ FE-miniloan-014 | รับเข้าแล้ว 2026-09-06T09:55:02Z |
| ADR-006 | design · revised | 2026-09-06T14:05:00Z | design ตัดสินเอง (ไม่มีใบเปลี่ยนแปลงจาก req) — dev เปิด GAP-miniloan-006 ระหว่าง /dev:plan: ENT-010 ประกาศ fieldName เป็น type: string อิสระ ไม่มีรายการค่าที่รับได้ และไม่มี artifact ใบไหน map ค่าของมันไปหา attribute ของเอนทิตีใดเลย · FE-miniloan-015 (ยื่นคำขอ) ไม่เดือดร้อนเพราะมันเก็บ oldValue/newValue เป็นข้อความล้วน แต่ FE-miniloan-016 ต้องเอาค่าใหม่ไปลงของจริงตาม AC-miniloan-076 ("ค่าบนบัญชีเปลี่ยนเป็นค่าใหม่ก็ต่อเมื่อผ่านขั้นที่สองแล้ว") และไม่มีอะไรให้มันใช้ลง ⇒ dev จะต้องมินต์รายการฟิลด์เอง ซึ่งเป็นการตัดสินว่าข้อมูลอะไรของบัญชีที่ปิดแล้วแก้ได้ ไม่ใช่รายละเอียดการเขียนโค้ด · ข้อเท็จจริงที่ชี้ขาดรูปของคำตอบ: ตัวอย่างของ AC-miniloan-076 เองคือ "ยอดชำระงวดสุดท้ายถูกบันทึกผิด" ซึ่งอยู่ที่ ENT-009 Payment.amount ไม่ใช่ attribute ใดของ ENT-006 เลย ขอบเขตของการปรับปรุงจึงเป็นบัญชี *และรายการชำระของบัญชีนั้น* ไม่ใช่ตัวบัญชีอย่างเดียว · การแก้: fieldName เปลี่ยนจาก string เป็น enum ที่ถือรายการปิดของชื่อฟิลด์แบบเต็ม (LoanAccount.closedAt · LoanAccount.closeReason · LoanAccount.assignedOperationsId · Payment.amount · Payment.recordedAt) และเพิ่ม attribute targetRecordId เพื่อระบุแถวที่ถูกแก้ เพราะ Payment ไม่ใช่ตัวบัญชีและ loanAccountId ชี้มันไม่ได้ · ENT-008 Installment ไม่อยู่ในรายการเลย และนั่นไม่ใช่การตัดสินใหม่ — BR-miniloan-045@v1 ล็อกตารางผ่อนของบัญชีที่ปิดแล้วถาวร และ AC-miniloan-108 พิสูจน์ไว้แล้วว่าถึงมีผู้อนุมัติก็แก้ไม่ได้ · ฟิลด์ที่ถูกกันออกจากรายการโดยมีกฎรองรับ: applicationId (unique constraint หนึ่งใบสมัครหนึ่งบัญชี), interestRateVersionId (BR-miniloan-037@v1 ผูกถาวร), status (BR-miniloan-021@v1 เป็นเจ้าของประตูปิดบัญชี) · ฟิลด์ที่ถูกกันออกด้วยเหตุผลที่ design เขียนเองรอบนี้ ไม่ใช่กฎที่ใครประกาศไว้ก่อน: principalAmount, outstandingPrincipal, termMonths, disbursedAt — หลักที่ใช้คือ "ปรับปรุงได้เฉพาะข้อเท็จจริงที่ถูกบันทึก ไม่ใช่ค่าที่คำนวณมาหรือค่าที่เป็นตัวตนของบัญชี" · สิ่งที่ ADR ใบนี้ไม่ตัดสินและยกเป็นการ์ดแยกไปหา req: เมื่อ Payment.amount ที่ปิดบัญชีถูกแก้ให้น้อยลง outstandingPrincipal ที่เป็น 0.00 กับ closeReason: FullyPaid หมายความว่าอะไร — BR-miniloan-021@v1 เป็นเจ้าของทั้งสองค่า และเป็นคำถามเชิงกฎ ไม่ใช่รูปของข้อมูล · ผลต่อโค้ดที่ลงไปแล้ว: FE-miniloan-015 commit 1687dd7 เก็บ fieldName เป็น String และไม่มี targetRecordId จึงต้องผ่าน /dev:revise · หน่วยที่ถูกบล็อกอยู่คือ FE-miniloan-016 | รับเข้าแล้ว 2026-09-06T14:20:00Z |
| ADR-007 | design · revised | 2026-09-07T02:30:00Z | design ตัดสินเอง (ไม่มีใบเปลี่ยนแปลงจาก req — change-set.json ทั้ง 5 ใบถูกบันทึกและ absorb ครบแล้ว) — dev เปิด GAP-miniloan-012 หลัง build FE-miniloan-018 และโต๊ะ /design:datamodel gaps เปิดเผยว่ามันไม่ใช่ช่องว่าง แต่เป็นข้อขัดแย้งระหว่างสองสิ่งที่เซ็นไปแล้ว · DQ-miniloan-004 ปิดที่โต๊ะนี้เมื่อ 2026-09-05 (node ACL-018) ด้วยคำตอบว่า "แดชบอร์ดใช้ขอบเขตข้อมูลเดียวกับการเปิดดูรายใบ คือ own ไม่ใช่ทั้งระบบ … นับเฉพาะใบสมัครและบัญชีที่ตัวเองถูกมอบหมาย" และคำตอบนั้นกลายเป็น ACL-018 (UC-miniloan-020) scope: own กับ ACL-030 (UI-miniloan-009) scope: own ใน rbac.json ที่ approved โดย aplus191 เมื่อ 2026-09-03T05:17:39Z · แต่ AC-miniloan-110 ซึ่งเป็นเกณฑ์รับของ req เขียน given ว่า "ระบบมีใบสมัครสถานะ Submitted 3 ใบ · UnderReview 2 ใบ · Approved 1 ใบ และมีบัญชีสินเชื่อสถานะ Active 5 บัญชี · Closed 4 บัญชี" แล้ว then ว่าแดชบอร์ดต้องแสดง 3 · 2 · 1 · 5 · 4 ครบทั้งห้าตัว — เป็นการนับทั้งระบบ ไม่มีคำว่ามอบหมายอยู่ในเกณฑ์รับเลยสักคำ · การตัดสิน: ACL-018 และ ACL-030 เปลี่ยนจาก scope: own เป็น scope: all และคำตอบเดิมของ DQ-miniloan-004 ถูกกลับ · หลักฐานสี่ข้อที่ชี้ขาด (1) ENT-006 LoanAccount ไม่มี attribute ใดผูกกับ Loan Officer เลย — สิบฟิลด์ของมันคือ applicationId · interestRateVersionId · principalAmount · outstandingPrincipal · termMonths · status · assignedOperationsId · disbursedAt · closedAt · closeReason และ assignedOperationsId เป็นของ Operations ซึ่ง DQ-miniloan-011 ระบุไว้แล้วว่าเป็น snapshot ณ วันเบิกจ่าย ⇒ ครึ่งหลังของคำตอบเดิม ("นับบัญชีที่ตัวเองถูกมอบหมาย") ไม่มีฟิลด์รองรับ และไม่เคยมี คำตอบที่สร้างไม่ได้ด้วยข้อมูลที่ตัวเองประกาศไว้ ไม่ใช่คำตอบ (2) BR-miniloan-024@v1 ซึ่งเป็นเจ้าของแดชบอร์ดเขียนแค่ "แดชบอร์ดของ Loan Officer แสดงจำนวนแยกตามสถานะ Submitted / UnderReview / Approved / Active / Closed โดยนับข้ามทั้ง LoanApplication และ LoanAccount" ไม่พูดถึงขอบเขตเลย และ provenance ที่ยกมาจากเอกสารลูกค้า (SRC-001 §7 · US-12) ก็ไม่พูดถึงเช่นกัน ⇒ ขอบเขตเป็นสิ่งที่ design เติมเข้าไปเอง ไม่ใช่สิ่งที่ลูกค้าสั่ง (3) UC-miniloan-020 mainFlow เขียนว่า "ระบบนับใบสมัครแยกตามสถานะ … และนับบัญชีสินเชื่อแยกตามสถานะ" ไม่มีคำว่าของตัวเอง และ alternateFlow เขียนว่า "ระบบยังไม่มีใบสมัครหรือบัญชีสินเชื่อเลยสักรายการ" ซึ่งเป็นภาษาของทั้งระบบ ⇒ use case เองก็อ่านเป็น all อยู่ก่อนแล้ว การเปลี่ยน ACL จึงทำให้ rbac.json กลับมาตรงกับ functions.json ไม่ใช่ทำให้ห่างออก (4) AC-miniloan-110 กับ AC-miniloan-111 เป็นของ req และ design ปฏิเสธเกณฑ์รับของ req เองไม่ได้ · สิ่งที่ ADR ใบนี้ไม่ได้เปลี่ยน: UC-miniloan-020 ไม่ขยับ (ข้อความของมันอ่านเป็น all อยู่แล้ว) · UI-miniloan-009 ไม่ขยับ (ฟิลด์ห้าช่องกับ states ของมันไม่ได้พูดถึงขอบเขต) · ACL-027 กับ ACL-028 ยังเป็น own ต่อไปและถูกต้องแล้ว เพราะ ENT-002 มี assignedLoanOfficerId จริง คิวและหน้าพิจารณาจึงมีฟิลด์รองรับ สิ่งที่ ADR ใบนี้กลับคือขอบเขตของ *แดชบอร์ด* ข้อเดียว ไม่ใช่หลักการ own ทั้งหมดของ ROLE-002 · ผลต่อโค้ดที่ลงไปแล้ว: ไม่มี — FE-miniloan-018 (commit b650d3c) สร้างด้วยตัวเลขทั้งระบบไปแล้วและได้จดเหตุผลไว้ใน DashboardSummaryService ว่าเป็นการอ่านแบบเดียวที่ทำให้ AC-miniloan-110 ผ่าน ADR ใบนี้ทำให้โค้ดนั้นมีต้นน้ำรองรับ แทนที่จะยืนอยู่ด้วยการตัดสินของ dev · การแก้ต้องผ่าน /design:approve --revoke rbac.json --because ADR-007 ก่อน เพราะ rbac.json เซ็นแล้วและข้อผูกพัน 7 ปฏิเสธการเขียนทับ | รับเข้าแล้ว 2026-09-07T03:59:08Z |
...
| datamodel.json | /design:datamodel | done | approved | aplus191 | 2026-09-06T14:55:02Z | — |
...
| rbac.json | /design:rbac | done | approved | aplus191 | 2026-09-07T03:59:08Z | — |
| nfr.json | /design:nfr | done | approved | aplus191 | 2026-09-03T05:17:39Z | — |
| sitemap.json | /design:sitemap | done | approved | aplus191 | 2026-09-05T00:00:00Z | — |
| modules/miniloan/screens.json | /design:sitemap | done | approved | aplus191 | 2026-09-06T14:57:54Z | — |
| interfaces.json | /design:interface | done | approved | aplus191 | 2026-09-06T09:55:52Z | — |
...
erDiagram
  ...
  ClosedAccountAdjustment {
    reference loanAccountId "บังคับ"
    string targetRecordId "บังคับ"
    enum fieldName "บังคับ"
...
| targetRecordId | — | string | บังคับ | ไม่ใช่ | — | — | id ของแถวที่ถูกแก้จริง — เมื่อ fieldName ขึ้นต้นด้วย LoanAccount. ค่านี้เท่ากับ loanAccountId เสมอ · เมื่อขึ้นต้นด้วย Payment. ค่านี้คือ id ของ ENT-009 ที่ต้องเป็นรายการชำระของบัญชีเดียวกันกับ loanAccountId · เก็บเป็น string ไม่ใช่ reference เพราะปลายทางเป็นได้สองเอนทิตี ซึ่ง type reference ที่ต้องระบุเอนทิตีเดียวรับไม่ได้ (ADR-006) |
| fieldName | — | enum: LoanAccount.closedAt · LoanAccount.closeReason · LoanAccount.assignedOperationsId · Payment.amount · Payment.recordedAt | บังคับ | ไม่ใช่ | — | — | ฟิลด์ที่ขอแก้ ระบุเป็นชื่อเต็ม <เอนทิตี>.<attribute> จากรายการปิดข้างต้น (ADR-006 · ตอบ GAP-miniloan-006) · ขอบเขตครอบคลุมทั้งตัวบัญชีและรายการชำระของบัญชีนั้น เพราะตัวอย่างของ AC-miniloan-076 เองคือการแก้ยอดชำระงวดสุดท้าย ซึ่งอยู่ที่ ENT-009 ไม่ใช่ ENT-006 · ENT-008 Installment ไม่อยู่ในรายการเลย เพราะ BR-miniloan-045@v1 ล็อกตารางผ่อนของบัญชีที่ปิดแล้วถาวร และ AC-miniloan-108 พิสูจน์ว่าถึงมีผู้อนุมัติก็ยังแก้ไม่ได้ · ฟิลด์ที่ถูกกันออกโดยมีกฎรองรับ: applicationId (หนึ่งใบสมัครหนึ่งบัญชี) · interestRateVersionId (BR-miniloan-037@v1 ผูกถาวร) · status (BR-miniloan-021@v1 เป็นเจ้าของประตูปิดบัญชี) · ฟิลด์ที่ถูกกันออกด้วยหลักที่ design เขียนเองใน ADR-006 ไม่ใช่กฎที่ใครประกาศไว้ก่อน: principalAmount · outstandingPrincipal · termMonths · disbursedAt — หลักนั้นคือ ปรับปรุงได้เฉพาะข้อเท็จจริงที่ถูกบันทึก ไม่ใช่ค่าที่คำนวณมาหรือค่าที่เป็นตัวตนของบัญชี |
| oldValue | — | string | บังคับ | ไม่ใช่ | — | — | ค่าเดิมของฟิลด์ที่ระบุ เก็บเป็นข้อความที่ render แล้ว ต้องเก็บไว้ไม่ถูกทับ ตาม BR-miniloan-041@v1 · ฟิลด์ที่เป็นจำนวนเงินใช้รูปสองตำแหน่งทศนิยมตาม BR-miniloan-035@v1 |
| newValue | — | string | บังคับ | ไม่ใช่ | — | — | ค่าใหม่ที่ขอแก้ เก็บเป็นข้อความที่ render แล้ว รูปเดียวกับ oldValue ตาม BR-miniloan-041@v1 |
...
| รายการที่ขอแก้ | ENT-010.targetRecordId | กรอก | บังคับ | ระบุแถวที่ถูกแก้ — เมื่อฟิลด์ที่เลือกเป็นของบัญชี ค่านี้คือบัญชีที่กำลังเปิดอยู่ · เมื่อเป็นของรายการชำระ ค่านี้คือรายการชำระของบัญชีเดียวกัน (ADR-006) · บังคับกรอกเมื่อยื่นคำขอปรับปรุงบัญชีที่ปิดแล้วตาม BR-miniloan-041@v1 | BR-miniloan-041@v1 |
| ชื่อฟิลด์ที่ขอแก้ | ENT-010.fieldName | กรอก | บังคับ | บังคับกรอกเมื่อยื่นคำขอปรับปรุงบัญชีที่ปิดแล้วตาม BR-miniloan-041@v1 · ค่าที่รับได้เป็นรายการปิดที่ ENT-010.fieldName ประกาศไว้ (ADR-006) — ไม่มีสำเนาของรายการนั้นอยู่บนหน้าจอนี้ | BR-miniloan-041@v1 |
...
flowchart LR
  ...
  SYS --- E24
  E25["API-026 · บันทึกการชำระยอดปิดบัญชีก่อนกำหนด"]
  SYS --- E25
```
...
| API-026 | บันทึกการชำระยอดปิดบัญชีก่อนกำหนด | POST /loan-accounts/{id}/payoff-settlement | Operations บันทึกการชำระยอดปิดบัญชีก่อนกำหนดเต็มจำนวนตามที่ API-015 คำนวณไว้ · เมื่อยอดครบ บัญชีเปลี่ยนเป็น Closed ด้วยเหตุผล EarlySettlement และงวดที่เหลือถูกยกเลิกตาม BR-miniloan-023@v1 · ยอดที่ไม่ตรงกับยอดปิดบัญชี ณ วันที่บันทึกถูกปฏิเสธทั้งรายการ ไม่รับชำระบางส่วนและไม่รับเกิน · แยกจาก API-014 เพราะ API-014 รับชำระ 'รายงวด' และคิดค่าธรรมเนียม 1% จากส่วนเกินตาม BR-miniloan-046@v2 ขณะที่ยอดปิดก่อนกำหนดคิด 1% จากเงินต้นคงเหลือตาม BR-miniloan-022@v1 — ฐานการคำนวณคนละตัว |
...
| BR-miniloan-015@v1 | domain · db | คำนวณตารางผ่อนที่ domain · ขอบเขตของ transaction คือทั้งคำสั่งเบิกจ่าย: เปลี่ยนสถานะใบสมัครเป็น Disbursed, สร้าง LoanAccount, สร้าง RepaymentSchedule และ Installment ทุกงวด อยู่ในธุรกรรมเดียวกันทั้งหมด ล้มเหลวที่จุดใดก็ตาม = rollback ทั้งก้อน ไม่มีบัญชีที่ไม่มีตารางผ่อน และไม่มีสถานะครึ่งทางให้ใครต้องมาซ่อมทีหลัง (ตอบ DQ-miniloan-008 · ADR-004) · กู้คืนด้วยการที่ผู้ใช้สั่งซ้ำเอง ไม่มี retry อัตโนมัติตาม BR-miniloan-042@v1 และ REQ-miniloan-006 · การสั่งซ้ำปลอดภัยเพราะ ENT-012 IdempotencyKey บังคับคู่ (commandType, requestId) ด้วย unique constraint ที่ db ซึ่งเป็นเหตุผลที่ db อยู่ในรายการนี้ ไม่ใช่แค่ domain · SCN-miniloan-035 และ SCN-miniloan-036 เป็นสองใบที่พิสูจน์ข้อนี้อยู่แล้ว |
...
| BR-miniloan-022@v1 | api · domain | สูตรคำนวณยอดปิดบัญชีก่อนกำหนด — domain คำนวณ (เงินต้นคงเหลือ + ดอกเบี้ยค้างจ่าย + ค่าธรรมเนียม 1% ของเงินต้นคงเหลือ) และ api ปฏิเสธการชำระที่ยอดไม่ตรงกับตัวเลขนั้น ณ วันที่บันทึก · ชั้น api ถูกเพิ่มพร้อม API-026 (ADR-005) เพราะก่อนหน้านี้ไม่มี endpoint ใดรับ UC-miniloan-016 ทำให้ ACL-014 ที่สั่ง enforceAt [api, domain] ไม่มีที่อยู่ให้บังคับได้จริง |
...
| ACL-018 | ROLE-002 | UC-miniloan-020 | ดูแดชบอร์ดภาพรวมสถานะ | all | — | api · domain |
...
| ACL-030 | ROLE-002 | UI-miniloan-009 | เปิดหน้าแดชบอร์ดภาพรวมสถานะ | all | — | api · domain |
...
| API-026 | ⛔ ปฏิเสธ | ⛔ ปฏิเสธ | ⛔ ปฏิเสธ | ⛔ ปฏิเสธ | ⛔ ปฏิเสธ |
